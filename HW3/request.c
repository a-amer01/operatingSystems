...
void requestError(QueueNode req, char *cause, char *errnum, char *shortmsg, char *longmsg)
{
    int fd =  req->data; // getFdRequest(req);
...
void requestServeDynamic(QueueNode req, char *filename, char *cgiargs)
{
    int fd = req->data; // getFdRequest(req);
    char buf[MAXLINE], *emptylist[] = {NULL};

    //increaseDynamicCount(req->thread_info);
    ++(req->thread_info->dynamic_count);
    ...
    sprintf(buf, "%sStat-Thread-Id:: %d\r\n", buf, req->thread_info->id);
    sprintf(buf, "%sStat-Thread-Count:: %d\r\n", buf, req->thread_info->count);
    sprintf(buf, "%sStat-Thread-Static:: %d\r\n", buf,req->thread_info->static_count);
    sprintf(buf, "%sStat-Thread-Dynamic:: %d\r\n", buf,req->thread_info->dynamic_count);
    Rio_writen(fd, buf, strlen(buf));
...
void requestServeStatic(QueueNode req, char *filename, int filesize)
{
    int fd = req->data; // getFdRequest(req);
    int srcfd;
    char *srcp, filetype[MAXLINE], buf[MAXBUF];

    //increaseStaticCount(req->thread_info);
     (req->thread_info->static_count)++;
...
// handle a request
void requestHandle(QueueNode req)
{
    int fd = req->data; // getFdRequest(req);
    int is_static;
    struct stat sbuf;
